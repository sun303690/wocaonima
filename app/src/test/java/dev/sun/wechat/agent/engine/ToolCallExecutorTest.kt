...
import dev.sun.wechat.agent.tool.ProviderKind
import dev.sun.wechat.agent.tool.ProviderTool
import dev.sun.wechat.agent.tool.ToolProvider
...
class ToolCallExecutorTest {
    ...
    fun `side-effect-free call runs directly in every level`() = runBlocking {
        for (level in PermissionLevel.entries) {
            ...
            assertEquals(dev.sun.wechat.agent.data.entity.ApprovalStatus.AUTO_ALLOWED, result.status)
    ...
    fun `auto edit gates other side-effecting tools manually`() = runBlocking {
        ...
        assertEquals(dev.sun.wechat.agent.data.entity.ApprovalStatus.USER_APPROVED, result.status)
    }
    ...
    fun `auto edit does not trust a remote tool named edit`() = runBlocking {
        ...
        assertEquals(dev.sun.wechat.agent.data.entity.ApprovalStatus.USER_REJECTED, result.status)
    ...
    fun `auto approval smart-approved call reports AI approval`() = runBlocking {
        ...
        assertEquals(dev.sun.wechat.agent.data.entity.ApprovalStatus.AI_APPROVED, result.status)
        assertEquals("pong", result.text)
    }
    ...
    fun `auto approval without small model fails closed and reports AI rejection`() = runBlocking {
        ...
        assertEquals(dev.sun.wechat.agent.data.entity.ApprovalStatus.AI_REJECTED, result.status)
    }
