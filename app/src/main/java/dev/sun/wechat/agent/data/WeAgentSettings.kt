...
import dev.sun.wechat.agent.data.WeAgentSettings.load
...
object WeAgentSettings {
    ...
    suspend fun sendWhileRunningMode(): dev.sun.wechat.features.api.agent.WeAgentService.SendWhileRunningMode =
        when (get(KEY_SEND_WHILE_RUNNING)) {
            "QUEUE_AS_STEER" -> dev.sun.wechat.features.api.agent.WeAgentService.SendWhileRunningMode.QUEUE_AS_STEER
            else -> dev.sun.wechat.features.api.agent.WeAgentService.SendWhileRunningMode.QUEUE_AFTER_TURN
        }
}
