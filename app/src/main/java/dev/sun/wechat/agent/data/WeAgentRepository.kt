package dev.sun.wechat.agent.data

import dev.sun.wechat.utils.fs.asPath
import androidx.room.withTransaction
import dev.sun.wechat.agent.data.entity.ConditionalPromptEntity
import dev.sun.wechat.agent.data.entity.ExternalServiceEntity
import dev.sun.wechat.agent.data.entity.MessageEntity
import dev.sun.wechat.agent.data.entity.MessageRole
import dev.sun.wechat.agent.data.entity.LinuxEnvironmentEntity
...
import dev.sun.wechat.agent.data.entity.PerTurnPromptEntity
import dev.sun.wechat.agent.data.entity.PresetPromptEntity
import dev.sun.wechat.agent.data.entity.ProviderEntity
import dev.sun.wechat.agent.data.entity.SessionEntity
import dev.sun.wechat.agent.data.entity.SystemPromptEntity
import dev.sun.wechat.agent.data.entity.ToolCallEntity
...
import dev.sun.wechat.agent.environment.LinuxEnvironmentType
import dev.sun.wechat.agent.environment.EnvironmentSnapshot
import dev.sun.wechat.agent.environment.LinuxEnvironmentDeletionPlan
import dev.sun.wechat.agent.environment.LinuxEnvironmentSessionState
import dev.sun.wechat.agent.environment.LinuxEnvironmentSessionTransition
...
object WeAgentRepository {
    ...
    suspend fun appendBridgeToolAudit(entry: dev.sun.wechat.agent.bridge.ToolBridgeSession.AuditEntry) {
        db.bridgeToolAuditDao().insert(
            dev.sun.wechat.agent.data.entity.BridgeToolAuditEntity(
    ...
    suspend fun announceEffectiveLinuxEnvironment(sessionId: String, environment: dev.sun.wechat.agent.environment.EnvironmentSnapshot, previousEnvironment: dev.sun.wechat.agent.environment.EnvironmentSnapshot? = null): Boolean =
        db.withTransaction {
    ...
    private fun environmentReminder(previous: dev.sun.wechat.agent.environment.EnvironmentSnapshot?, environment: dev.sun.wechat.agent.environment.EnvironmentSnapshot): String =
     ...
     */
    suspend fun updateSessionUsage(id: String, usage: dev.sun.wechat.agent.model.LlmUsage?) {
        ...
        toolCalls.forEach { tc ->
            ...
            db.toolCallDao().upsert(
                ToolCallEntity(
                    ...
                    resultJson = null,
                    approvalStatus = dev.sun.wechat.agent.data.entity.ApprovalStatus.AUTO_ALLOWED,
                    approvalReason = null,
                    executedAt = null,
    ...
    suspend fun appendToolResult(
        ...
        resultText: String,
        status: dev.sun.wechat.agent.data.entity.ApprovalStatus,
    ) {
    ...
    suspend fun deleteModelProvider(id: String) {
        ...
        dev.sun.wechat.agent.model.ModelProviderManager.invalidate(id)
    ...
    private fun notifyModelsDeleted(modelIds: Set<String>) {
        ...
        runCatching {
            dev.sun.wechat.features.api.agent.WeAgentService.onModelsDeleted(modelIds)
        }
    }
    ...
    suspend fun upsertMcpProvider(provider: ProviderEntity) {
        ...
        dev.sun.wechat.agent.mcp.McpClientManager.sync()
    }
    ...
    suspend fun deleteSystemPrompt(id: String) {
        ...
        settingKeys.forEach { WeAgentSettings.clearCached(it) }
        runCatching { dev.sun.wechat.features.api.agent.WeAgentService.onSystemPromptDeleted(id) }
    }
    ...
    suspend fun deleteLinuxEnvironment(
        ...
        db.withTransaction {
            ...
            db.settingDao().upsert(
                dev.sun.wechat.agent.data.entity.SettingEntity(
    ...
    private fun validateLinuxEnvironment(environment: LinuxEnvironmentEntity) {
        ...
        dev.sun.wechat.agent.environment.LinuxEnvironmentManager.parseEnvironmentVariables(
        ...
        when (environment.type) {
            LinuxEnvironmentType.PROOT, LinuxEnvironmentType.CHROOT -> {
                ...
                if (environment.type == LinuxEnvironmentType.CHROOT) {
                    dev.sun.wechat.agent.environment.ArchLinuxInstanceLayout.validatePublishedRootfs(
    ...
    fun observeTriggers(): Flow<List<dev.sun.wechat.agent.data.entity.TriggerEntity>> =
        db.triggerDao().observeAll()

    suspend fun getAllTriggersOnce(): List<dev.sun.wechat.agent.data.entity.TriggerEntity> =
        db.triggerDao().getAllOnce()

    suspend fun getEnabledTriggers(): List<dev.sun.wechat.agent.data.entity.TriggerEntity> =
        db.triggerDao().getEnabled()

    suspend fun getTrigger(id: String): dev.sun.wechat.agent.data.entity.TriggerEntity? =
        db.triggerDao().getById(id)

    suspend fun getTriggersForSession(sessionId: String): List<dev.sun.wechat.agent.data.entity.TriggerEntity> =
        db.triggerDao().getForSession(sessionId)

    suspend fun upsertTrigger(trigger: dev.sun.wechat.agent.data.entity.TriggerEntity) =
