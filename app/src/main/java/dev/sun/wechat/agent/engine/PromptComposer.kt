...
class PromptComposer(
    ...
    private fun defaultSystemPrompt(): String {
        ...
        return buildString {
            ...
            if (environment.type == dev.sun.wechat.agent.environment.LinuxEnvironmentType.PROOT || environment.type == dev.sun.wechat.agent.environment.LinuxEnvironmentType.CHROOT) {
                append("\n- 本地 Arch Linux 共享 Android 内核；${if (environment.type == dev.sun.wechat.agent.environment.LinuxEnvironmentType.PROOT) "PRoot 无内核级隔离" else "chroot 不是安全边界"}。")
            }
            append(
                """
