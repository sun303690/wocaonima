...
import dev.sun.wechat.agent.model.LlmToolSpec
...
import dev.sun.wechat.agent.tool.WireTool
...
class TurnConfig(
    ...
    val toolVisibility: dev.sun.wechat.agent.tool.ToolVisibility =
        dev.sun.wechat.agent.tool.ToolVisibility.fromGlobals(),
    /**
...
class AgentSessionEngine(
    ...
    fun runTurn(
        ...
        try {
            ...
            while (true) {
                ...
                val request = dev.sun.wechat.agent.model.LlmRequest(
    ...
    private fun teardownTurn() {
        runCatching { dev.sun.wechat.agent.ui.UiAutomator.cancelActiveGesture() }
            .onFailure { WeLogger.w(TAG, "gesture teardown failed: ${it.message}") }
        runCatching { dev.sun.wechat.agent.jvm.JvmObjectRegistry.purgeDead() }
    ...
    private fun parseArgs(argumentsJson: String): JsonObject =
        runCatching { dev.sun.wechat.agent.model.LlmJson.json.parseToJsonElement(argumentsJson).jsonObject }
