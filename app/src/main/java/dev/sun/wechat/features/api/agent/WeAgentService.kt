package dev.sun.wechat.features.api.agent
...
import dev.sun.wechat.agent.engine.ApprovalGateway
import dev.sun.wechat.agent.engine.ManualApprovalHandler
import dev.sun.wechat.agent.engine.ManualApprovalResult
import dev.sun.wechat.agent.engine.PendingApproval
import dev.sun.wechat.agent.engine.PromptComposer
import dev.sun.wechat.agent.engine.SmallModelRef
import dev.sun.wechat.agent.engine.TurnConfig
import dev.sun.wechat.agent.engine.ToolCallExecutor
import dev.sun.wechat.agent.bridge.ToolBridgeServer
import dev.sun.wechat.agent.environment.LinuxEnvironmentManager
import dev.sun.wechat.agent.environment.NATIVE_ENVIRONMENT_ID
import dev.sun.wechat.agent.environment.ProotEnvironmentCreationResult
import dev.sun.wechat.agent.environment.ChrootEnvironmentCreationResult
import dev.sun.wechat.agent.terminal.EnvironmentTerminalBackend
import dev.sun.wechat.agent.terminal.SshTerminalBackend
import dev.sun.wechat.agent.terminal.TerminalManager
import dev.sun.wechat.agent.mcp.McpClientManager
import dev.sun.wechat.agent.model.LlmToolCall
import dev.sun.wechat.agent.model.ModelProviderManager
import dev.sun.wechat.agent.model.local.LocalLlamaModels
import dev.sun.wechat.agent.model.local.LocalLlamaSync
import dev.sun.wechat.agent.net.ExternalServiceId
import dev.sun.wechat.agent.tool.BuiltinToolProvider
import dev.sun.wechat.agent.tool.PermissionLevel
import dev.sun.wechat.agent.tool.ToolRegistry
import dev.sun.wechat.agent.trigger.TriggerManager
import dev.sun.wechat.agent.ui.UiImageSink
import dev.sun.wechat.features.api.agent.WeAgentService.ballState
import dev.sun.wechat.features.api.agent.WeAgentService.handleEvent
import dev.sun.wechat.features.api.agent.WeAgentService.init
import dev.sun.wechat.features.api.agent.WeAgentService.messageListState
import dev.sun.wechat.features.api.agent.WeAgentService.messageListSyncedSessionId
import dev.sun.wechat.features.api.agent.WeAgentService.newSession
import dev.sun.wechat.features.api.agent.WeAgentService.pendingApproval
import dev.sun.wechat.features.api.agent.WeAgentService.pendingApprovals
import dev.sun.wechat.features.api.agent.WeAgentService.resolveTurnConfig
import dev.sun.wechat.features.api.agent.WeAgentService.runTurn
import dev.sun.wechat.features.api.agent.WeAgentService.runningTurns
import dev.sun.wechat.features.api.agent.WeAgentService.sendMessage
import dev.sun.wechat.features.api.agent.WeAgentService.switchSession
import dev.sun.wechat.features.api.agent.WeAgentService.syncForeground
import dev.sun.wechat.features.api.agent.WeAgentService.uiMessages
import dev.sun.wechat.features.api.agent.WeAgentService.uiSessions
...
object WeAgentService : TriggerManager.TriggerHost {
    ...
    val currentUsage = mutableStateOf<dev.sun.wechat.agent.model.LlmUsage?>(null)
    ...
    data class ChatRow(
        ...
        /** Persisted [createdAt] from [dev.sun.wechat.agent.data.entity.MessageEntity]; [java.time.Instant.EPOCH] for live-streaming rows. */
     ...
     * or null if no model is configured. [dev.sun.wechat.agent.trigger.TriggerManager.TriggerHost] impl.
     */
    ...
    private suspend fun switchSessionInternal(id: String) {
        ...
        val foreground = withContext(Dispatchers.Main) {
            ...
            currentUsage.value = session?.let {
                ...
                else dev.sun.wechat.agent.model.LlmUsage(it.promptTokens, it.completionTokens, it.totalTokens)
            }
    ...
    private suspend fun buildEngine(
        ...
        environment: dev.sun.wechat.agent.environment.EnvironmentSnapshot,
    ): AgentSessionEngine {
        val composer = PromptComposer(
            toolLoadingMode = WeAgentSettings.toolLoadingMode(),
            environment = environment,
            skillCatalog = dev.sun.wechat.agent.skill.SkillStore.enabledSkills().map { it.name to it.description },
    ...
    private suspend fun requestHighRiskApproval(operation: String, environment: dev.sun.wechat.agent.environment.EnvironmentSnapshot?): Boolean {
    ...
    private suspend fun resolveTurnConfig(sessionId: String): TurnConfig? {
        ...
        val toolVisibility = dev.sun.wechat.agent.tool.ToolVisibility(
